...
public class AppController {
    ...
    @PostMapping("/")
    public String signIn(@ModelAttribute SignInForm signInForm, Model model, RedirectAttributes redirectAttributes) {

        loggedUser = trainingService.findPerson(signInForm.getEmail(), signInForm.getPassword());
    ...
    public String addNewPerson(@Valid Person person, BindingResult bindingResult, RedirectAttributes redirectAttributes) throws IOException {
        ...
        try {
            ...
            redirectAttributes.addFlashAttribute("error", "Person with email address: " + person.getEmailAddress() + " is already taken");
    ...
    public String deletePersonById(@ModelAttribute FindPersonForm findPersonForm, RedirectAttributes redirectAttributes) {
        Person retrievedPerson = trainingService.findPerson(findPersonForm.getPersonId());
        if(retrievedPerson == null) {
            redirectAttributes.addFlashAttribute("error", "Person with ID: " + findPersonForm.getPersonId() + " does not exist");
        } else if(loggedUser != null && retrievedPerson.getEmailAddress().equals(loggedUser.getEmailAddress())) {
